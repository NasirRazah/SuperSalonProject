...
begin
  ...
  if @CloseoutType = 'E' or @CloseoutType = 'T' begin
    ...
    while @@fetch_status = 0 begin
	  ...
	  if @PType = 'I' and @TransType = 1  /* gift aid */
      begin
        set @GiftAid = @GiftAid + @Pr*@Qty;
      end 
      ...
      if @PType <> 'A' and @PType <> 'G' and @PType <> 'I' and @PType <> 'O' and @PType <> 'C' and @PType <> 'Z' and @PType <> 'H' and @PType <> 'S' and (@PType <> 'X' and @i_ProductID <> 111 and @i_SKU <> 'MGC') and @TransType = 1  /* product sales */
      begin
        ...
        begin
          ...
          if @Status = 3 begin /* layaway sales */
             ...
             while @@fetch_status = 0 begin
				 ...
				 if @PType = 'I' and @TransType = 1  /* gift cert sales */
			     begin
			       set @GiftAid = @GiftAid + @Pr*@Qty;
		         end   
                ...
                if @PType <> 'A' and @PType <> 'G' and @PType <> 'I' and @PType <> 'O' and @PType <> 'C' and @PType <> 'Z' and @PType <> 'S'  and @PType <> 'H' and (@PType <> 'X' and @i_ProductID <> 111 and @i_SKU <> 'MGC') and @TransType = 1  /* product sales */
                begin
  ...
  */
  ...
  where tr.CloseOutID = @CloseoutID and tt.name='Store Credit' and inv.ID not in ( select invoiceno from VoidInv);
    ...
    insert into CloseoutReportMain (TaxedSales,NonTaxedSales,Tax1Amount,Tax2Amount,Tax3Amount,ServiceSales,
				...
				DGCsold,PLGCsold,Free_Qty,Free_Amount,LottoPayout, GiftAid) 
				values
				...
				@BottleRefund,@DGCsold,@PLGCsold,@FreeQty,@FreeAmount,@LottoPayout,@GiftAid)  
  ...
  if @CloseoutType = 'C' begin
    ...
    while @@fetch_status = 0 begin
	  ...
	  if @PType = 'I' and @TransType = 1  /* gift aid sales */
      begin
  ...
  */
  ...
  and tt.name='Store Credit' and inv.ID not in ( select invoiceno from VoidInv);
    ...
    insert into CloseoutReportMain (TaxedSales,NonTaxedSales,Tax1Amount,Tax2Amount,Tax3Amount,ServiceSales,
				...
				BottleRefund,DGCsold,PLGCsold,Free_Qty,Free_Amount,LottoPayout,GiftAid) 
				values
                (@TaxedSales,@NonTaxedSales,@TaxAmt1,@TaxAmt2,@TaxAmt3,@ServiceSales,
				@ProductSales,@OtherSales,@DiscountItemNo,@DiscountItemAmount,@DiscountInvoiceNo,
				@DiscountInvoiceAmount,@LayawayDeposits,@LayawayRefund,@LayawayPayment,@LayawaySalesPosted,
