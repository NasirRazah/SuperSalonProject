/*
 USER CLASS : Report - KIT Item Listing
 */
using System;
...
{
    public partial class repKitReport : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        private void rExtCost_PrintOnPage(object sender, PrintOnPageEventArgs e)
        {
            if (intDecimalPlace == 3) 
                rExtCost.Text = GeneralFunctions.fnDouble(rExtCost.Text).ToString("f3");
            else
            {
        ...
        {
            if (intDecimalPlace == 3) 
                rExtRetail.Text = GeneralFunctions.fnDouble(rExtRetail.Text).ToString("f3");
            else
            {
